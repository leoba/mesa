...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="tei:TEI">
		...
		<xsl:for-each
			select="tei:teiHeader/tei:fileDesc/tei:sourceDesc/tei:msDesc/tei:msContents/tei:msItem[not(tei:msItem)] | tei:teiHeader/tei:fileDesc/tei:sourceDesc/tei:msDesc/tei:msContents/tei:msItem/tei:msItem ">
			...
			<xsl:variable name="locus">
				<xsl:value-of select="descendant::tei:locus[position() = 1]/@from"/>
			</xsl:variable>
			
			<xsl:variable name="folio">
				<xsl:value-of select="concat('fol. ',$locus)"></xsl:value-of>
			</xsl:variable>
			...
			<xsl:result-document href="{$outputDir}/{$filename}.rdf">
				...
				<rdf:RDF>
					...
					<walters:wam rdf:about="{$baseURL}/ManuscriptDescriptions/{$ms-id}/{$itemID}">
						...
						<xsl:value-of select="$newline"/>
						<rdfs:seeAlso rdf:resource="{$baseURL}/html/{$ms-id}/"/>
						<xsl:value-of select="$newline"/>
	...
	<xsl:template match="tei:msItem">
		...
		<dc:title>
				...
				<xsl:if
					test="not(ancestor::tei:msItem)">
					<xsl:value-of
						select="tei:title[@type='work']"
					/>, <xsl:value-of select="$sigla"/>, <xsl:value-of select="$folio"/>
				</xsl:if>
				<xsl:value-of
						select="../tei:title[@type='work']"
	...
	<xsl:template name="hasParts">
		<xsl:for-each select="//tei:msItem">
			...
			<dcterms:hasPart rdf:resource="{$baseURL}/ManuscriptDescriptions/{$ms-id}/{$itemID}"/>
