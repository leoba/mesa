...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="dataroot">
    ...
    <xsl:for-each select="//tbexMSDetails">

      <xsl:variable name="repository">British Library</xsl:variable>
      <xsl:variable name="datestart" select="tbexMSParts/StartDate"/>
      <xsl:variable name="dateend" select="tbexMSParts/EndDate"/>
      <xsl:variable name="date" select="tbexMSParts/Dates"/>
      ...
      <xsl:result-document href="{$outputDir}/{$filename}.rdf">
        <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
          ...
          <bl:MSPart xmlns:bl="http://www.bl.uk/catalogues/illuminatedmanuscripts/mesa/ns/"
            ...
            <xsl:if test="ancestor::tbexMSParts/Scribe">
              <role:SCR>
            ...
            <xsl:if test="contains($Form,'codex')">
