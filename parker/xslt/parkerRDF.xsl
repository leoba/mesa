...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="tei:TEI/tei:text/tei:body">
		...
		<xsl:for-each select="tei:msDesc">
			...
			<xsl:result-document href="{$outputDir}/{$filename}.rdf">
				...
				<rdf:RDF>
					<xsl:value-of select="$newline"/>
					<parker:pkr rdf:about="{$baseURL}uri/{$id}">
						<xsl:value-of select="$newline"/>
						...
						<collex:text>
							<xsl:value-of select="tei:p[@function='Codicology']"/>
							<xsl:value-of select="tei:p[@function='Additions']"/>
						</collex:text>
						<xsl:value-of select="$newline"/>
						<rdfs:seeAlso rdf:resource="{$baseURL}actions/page_turner.do?ms_no={$id}"/>
						<xsl:value-of select="$newline"/>
						<xsl:for-each select="tei:msContents/tei:msItem">
							<dcterms:hasPart>
								<xsl:attribute name="rdf:resource">
									<xsl:value-of select="$baseURL"/>uri/<xsl:value-of select="$id"/>-<xsl:value-of select="@n"
									/>.rdf</xsl:attribute>
							</dcterms:hasPart>
						</xsl:for-each>
						<xsl:value-of select="$newline"/>
	...
	<xsl:template match="tei:msDesc">
		...
		<xsl:for-each select="tei:p[@function='Provenance']">
			<dc:provenance>
				<xsl:value-of select="."/>
			</dc:provenance>
		</xsl:for-each>
		...
		<xsl:if test="tei:msContents/tei:textLang/@otherLangs">
			<xsl:for-each select="tokenize(tei:msContents/tei:textLang/@otherLangs,' ')">
				<dc:language>
					<xsl:value-of select="."/>
				</dc:language>
			</xsl:for-each>
		...
		<!-- publisher -->
		<role:PBL>
			<xsl:value-of select="tei:msIdentifier/tei:settlement"/>, <xsl:value-of
				select="tei:msIdentifier/tei:institution"/>, <xsl:value-of
...
<xsl:choose>
	...
	<!--<xsl:template name="hasParts">
		<xsl:for-each select="//tei:msContents/tei:msItem">
			<dcterms:hasPart>
				<xsl:attribute name="rdf:resource">
					<xsl:value-of select="tei:msIdentifier/tei:idno"/>-<xsl:value-of select="@n"/>.rdf
				</xsl:attribute>
			</dcterms:hasPart>
		</xsl:for-each>
	</xsl:template>-->
